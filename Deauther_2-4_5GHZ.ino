//Kho lưu trữ này được cung cấp "nguyên trạng", không có bất kỳ bảo đảm nào, dù rõ ràng hay ngụ ý, bao gồm nhưng không giới hạn ở các bảo đảm về tính thương mại, sự phù hợp cho một mục đích cụ thể hoặc không vi phạm quyền lợi. Trong mọi trường hợp, tác giả hoặc chủ sở hữu bản quyền sẽ không chịu trách nhiệm với bất kỳ yêu cầu, thiệt hại hoặc trách nhiệm nào khác, dù trong hợp đồng, lỗi dân sự hay cách khác, phát sinh từ, liên quan đến, hoặc kết nối với phần mềm hoặc việc sử dụng, khai thác phần mềm. Việc sử dụng kho lưu trữ này và nội dung bên trong là hoàn toàn tự nguyện và chịu rủi ro từ phía người dùng. Tác giả không đảm bảo tính chính xác, đáng tin cậy hoặc đầy đủ của bất kỳ thông tin nào mặc c cung cấp trong kho lưu trữ này. Bằng việc sử dụng kho lưu trữ này, bạn đồng ý với các điều khoản trên và xác nhận rằng bạn tự chịu trách nhiệm tuân thủ mọi quy định pháp luật hoặc quy tắc có liên quan. CÓ NGHĨA LÀ KHI BẠN SỬ DỤNG ĐOẠN MÃ ĐƯỢC VIẾT Ở TRÊN LÀ BẠN HOÀN TOÀN CHỊU TRÁCH NHIỆM, VIỆC BẠN SỬ DỤNG SẼ KHÔNG LIÊN QUAN ĐẾN NGƯỜI PHÁT HÀNH ĐOẠN MÃ TRÊN !
...
char *ssid = "Deauther_2-4_5GHZ";//Tên SSID
char *pass = "123456789";//MK SSID
...
for (size_t i = 0; i < scan_results.size(); i++) {
  // Đặt mạng đầu tiên là mạng được chọn mặc định
  if (i == 0) {
...
void startDeauth(int network_num) {
  ...
  /*Bạn liên hệ với mình để Mua đầy đủ bản code này nhá , Cảm Ơn Bạn*/
...
void handleDeauth(WiFiClient &client, String &request) {
    ...
    if (!deauth_targets.empty()) {
        deauth_running = true;
        deauth_start_time = millis(); // Ghi lại thời gian bắt đầu
...
response += "<style>";
response += "body { font-family: Arial, sans-serif; text-align: center; margin: 0; padding: 0; background-color: #fffacd; }"; // Màu nền vàng nhạt
...
if (deauth_running) {
    ...
    html += "</table>";
    html += "<p><strong>Thời gian bắt đầu:</strong> " + String(millis() / 1000) + " giây từ khi khởi động.</p>";
} else {
